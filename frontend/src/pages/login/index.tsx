          ...
          >
            {Array.isArray(appConfig.login_type) && (
              <TabsList className="mb-10">
                <TabsTrigger value={LoginTypeEnum.WECHAT}>Scan the WeChat QR code to log in</TabsTrigger>
                <TabsTrigger value={LoginTypeEnum.PASSWORD}>Login with account and password</TabsTrigger>
              </TabsList>
            )}
            <img src="https://lizzyai.com/favicon.ico"  className="mb-4 w-40 rounded-full" />
            ...
            <TabsContent value={LoginTypeEnum.WECHAT} className="flex w-full flex-col items-center">
              <Button className="mb-4 mt-12 w-[70%]" disabled={!protocolChecked} onClick={handleLogin}>
                {isWeixinBrowser ? 'WeChat login' : 'WeChat scan code to log in'}
              </Button>
            </TabsContent>
          ...
          {loginType === LoginTypeEnum.PASSWORD && (
            <div className="absolute bottom-3 right-3">
              ...
              <RetrievePasswordDialog>
                <Button variant={'ghost'}>Retrieve password</Button>
              </RetrievePasswordDialog>
            </div>
          )}
        </div>
